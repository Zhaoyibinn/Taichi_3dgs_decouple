...
with open('/home/zhaoyibin/3DRE/3DGS/taichi_3d_gaussian_splatting_complex/result/购买4/csv/apperance.txt', 'r') as file:
    content = file.read()
apperance = []
for i in content.split("\n"):
    if i =='':
...
with open('/home/zhaoyibin/3DRE/3DGS/taichi_3d_gaussian_splatting_complex/result/购买4/csv/3DGS_lightinstance.txt', 'r') as file:
    content = file.read()
GS_lightinstance = []
for i in content.split("\n"):
    if i =='':
...
# combined_df = pd.concat([data_pd_2DGS,data_pd_colmap,data_pd_ours], axis=1)
